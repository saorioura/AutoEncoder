...
class AutoEncoder:
    def __init__ (self,):
        ...
        conv1 = tf.layers.conv2d(self.X, 32, 5, strides=(2,2), padding = 'SAME', activation=tf.nn.relu, name='conv1')
        #bn1 = tf.layers.batch_normalization(conv1, training=self.is_training)
        #pool1 = tf.layers.max_pooling2d(conv1, 2, 2)
        conv2 = tf.layers.conv2d(conv1, 64, 3, strides=(2,2), padding='SAME', activation=tf.nn.relu, name='conv2')
        #bn2 = tf.layers.batch_normalization(conv2, training=self.is_training)
        #pool2 = tf.layers.max_pooling2d(conv2, 2, 2)
        # decoder
        # 7x7x64 14x14x32 28x28x1
        deconv1 = tf.layers.conv2d_transpose(conv2, 32, 3, strides=(2,2), padding='SAME', activation=tf.nn.relu)
        self.out = tf.layers.conv2d_transpose(deconv1, 1, 5, strides=(2,2), padding='SAME', activation=tf.nn.relu)
    ...
    def trainer(self):
        loss = tf.reduce_mean(tf.nn.l2_loss(self.out - self.X))
        ...
        with tf.Session() as sess:
            ...
            for i in range(num_epoch):
                ...
                for step in range(step_size):
                    ...
                    _, sloss = sess.run([train_op, summary_loss], feed_dict={self.X:data})

                    if global_step % 200 == 0:
                        aeimg, currentloss = sess.run([self.out, loss], feed_dict={self.X:data})
                        print(currentloss)
                    ...
                    if global_step % 200 == 0:
                        saver.save(sess, './AEout/logs/model.ckpt')
